...
bot(
	...
	async (message, match) => {
		...
		if (!update.length) return await message.send('*Levanter bot is up-to-date.*')
		await message.send(
			`${update.length} updates\n\n${update.join('\n').trim()}`
		)
		// return await message.send(
		// 	await genButtonMessage(
		// 		[{ id: 'updatenow', text: 'UPDATE NOW' }],
		// 		`*Updates*\n${update.join('\n').trim()}`,
		// 		`${update.length} updates`
		// 	),
		// 	{},
...
bot(
	{
		pattern: 'updatenow$',
		fromMe: true,
		desc: 'To-Update Levanter bot.',
		type: 'heroku',
	},
	async (message, match) => {
		const isupdate = await isUpdate()
		if (!isupdate.length)
			return await message.send('*Levanter bot is up-to-date.*\n*Nothing to Update.*')
