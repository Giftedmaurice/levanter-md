...
bot(
	...
	async (message, match) => {
		...
		if (!match && !welcome)
			return await message.send('*Example : welcome Hello &mention*')
		if (!match) {
			await message.send(welcome.message)
			const onOrOff = welcome && welcome.enabled ? 'on' : 'off'
			await message.send(
				`Welcome is ${onOrOff}\n\nhttps://github.com/Giftedmaurice/levanter-bot/wiki/Greetings`
			)
			...
			// 				urlButton: {
			// 					text: 'Example',
			// 					url: 'https://github.com/Giftedmaurice/levanter-bot/wiki/Greetings',
			// 				},
			// 			},
		...
		if (match == 'on' || match == 'off') {
			if (!welcome) return await message.send('*Example : welcome Hello #mention*')
...
bot(
	...
	async (message, match) => {
		...
		if (!match) {
			...
			const onOrOff = welcome && welcome.enabled ? 'on' : 'off'
			return await message.send(
				`Goodbye is ${onOrOff}\n\nhttps://https://github.com/Giftedmaurice/levanter-bot/wiki/Greetings`
			)
